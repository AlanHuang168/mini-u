...
# 用户输入 DeepSeek Key
deepseek_key = st.text_input("DeepSeek API Key (可选)", type="password", value=os.getenv("DEEPSEEK_API_KEY", ""))
...
if st.button("🚀 开始分析"):
    if not deepseek_key:
        ...
        if len(tickers) > 1:
            ...
            report_html = compare_stocks(tickers)
            st.success("✅ 多股票对比完成！")
        else:
        ...
        st.html(report_html)   # ← 改成 st.html（推荐）
        ...
        for i, ticker in enumerate(tickers):
            ...
            for role in roles:
                comment = get_role_comment(ticker, current_data, role)
